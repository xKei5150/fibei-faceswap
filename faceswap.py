...
def face_swap(source_image, target_face, swapper):
    return swapper.get(source_image, target_face, paste_back=True)
...
def process_frame(frame, app, swapper, upscaler):
    faces = face_detection(frame, app)
    processed_frames = []
    for face in faces:
        swapped_face = face_swap(frame, face, swapper)
        upscaled_frame = face_upscale(swapped_face, upscaler)
        processed_frames.append(upscaled_frame)
    return processed_frames


def process_video(video_path, output_video_path, app, swapper, upscaler):
    ...
    try:
        while cap.isOpened():
            ...
            processed_frames = process_frame(frame, app, swapper, upscaler)
...
def main():
    ...
    process_video(video_path, output_video_path, app, swapper, upscaler)
